...
// We keep the fetched area tightly matched to the circular platform
// in the scene (radius ≈ 150m, same as the ground disc).
const PLATFORM_RADIUS_METERS = 150;

export async function updateBuildings(lat, lng, radius = PLATFORM_RADIUS_METERS) {
  console.log(`[Buildings] Fetching OSM near ${lat}, ${lng} (r=${radius}m)...`);
...
async function fetchWithMirrors(query, attempt = 0) {
  ...
  try {
    ...
    console.warn(
      `[Buildings] Mirror fail: ${err.message}. Retrying mirror ${currentMirrorIndex} in ${delay / 1000}s...`,
    );
...
function processOSMData(data, refLat, refLng) {
  ...
  data.elements.forEach((el) => {
    if (el.type === 'way' && el.tags && el.tags.building) {
      ...
      // Cull buildings whose footprint lies entirely outside the platform radius
      const withinPlatform = points.some((p) => {
        const r = Math.sqrt(p.x * p.x + p.z * p.z);
        return r <= PLATFORM_RADIUS_METERS;
      });

      if (withinPlatform && points.length > 2) {
...
function createBuilding(points, tags) {
  ...
  const height =
    ...
    (parseFloat(tags['building:levels']) || 2) * 3 ||
    (Math.random() * 10 + 10);
  ...
  const color =
    BUILDING_COLOR_PALETTE[Math.floor(Math.random() * BUILDING_COLOR_PALETTE.length)];
